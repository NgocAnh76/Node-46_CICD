...
export const chatService = {
  ...
  listUserChat: async (req) => {
    ...
    search = search || ``;
    notMe = !!notMe; // hoặc có thể viết logic như sau: notMe = notMe === `true` ? true : false
    ...
    const users = await prisma.users.findMany({
      ...
      skip: skip,
      orderBy: {
        created_at: `desc`, // sắp xếp giảm dần: đưa video mới tạo ra lên đầu
      },
      where: where,
    });
    ...
    return {
      page, // trang hiện tại
      pageSize, // kích thước item trong 1 page: 10video trong một page
      totalPage, // tổng cộng có bnhiu trang
